...
3. Push to `main`. After **CI** succeeds on that push, the **Deploy to GitHub Pages** workflow builds the same commit (`npm ci`, `npm run build`), copies `dist/index.html` to `dist/404.html`, and publishes `dist`. You can also run **Deploy to GitHub Pages** manually via **Actions → Run workflow**. **Supabase keepalive** is scheduled from the default branch as well; it only performs the health ping when both Supabase secrets above are set (otherwise it skips).
